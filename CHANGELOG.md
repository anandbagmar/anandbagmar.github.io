...
- Rewrote internal Blogspot cross-links in blog posts to local paths so clicks stay on-site instead of bouncing through the Blogspot→site redirect. 70 of 74 `essenceoftesting.blogspot.(com|in)/YYYY/MM/slug.html` post links now point at `/blog/<slug>/` (mapped via each post's `redirect_from` + filename slug, with a stop-word-aware fallback for Blogger's article-dropping truncation); the 4 remaining point at posts that were never migrated and were left as-is. All 71 `/search/label/<tag>` links were repointed to `/blog/tags/#<tag>`.
- Fixed `jekyll serve` leaving local search broken: added a `_plugins/search_index.rb` `post_write` hook that runs `scripts/build-search-index.js` after every Jekyll build, so `_site/search-index.json` is always regenerated (including on incremental serve rebuilds). Previously only `./server.sh` or the CI `npm run build:search` step produced it, so a bare `bundle exec jekyll serve` served a site whose search index was missing. The hook degrades gracefully (warns, doesn't fail) when Node/lunr aren't present yet, so CI is unaffected.
- Removed non-content resource files from `sitemap.xml`: the `site.pages` loop in `_includes/sitemap_collection.xml` was listing `atom.xml`, `feed.xml`, `humans.txt`, `robots.txt`, `search.json`, `redirects.json`, and even `sitemap.xml` itself as crawlable `<loc>` URLs (7 junk entries wasting crawl budget). Added a guard so only real HTML pages are emitted — a URL must end in `/` (directory) or `.html`. Blog posts, content pages, `/blog/`, and `/blog/tags/` are unaffected; sitemap dropped from 270 to 263 entries.
- Fixed the search page (and the mobile-nav arrow/cleanup script) in CI/production: the production build runs the `compress.html` layout, which collapses newlines inside inline `<script>` blocks. Both scripts used `//` line comments, so once newlines were stripped a comment swallowed the rest of the script ("Unexpected end of input") and it never ran — the search index never loaded and `__searchReady` stayed undefined. Moved both into external files (`/assets/js/search.js`, `/assets/js/navigation.js`), which `compress.html` does not process. Reproduced only under `JEKYLL_ENV=production`; local `jekyll serve` runs in development where compression is off, which is why it was CI-only.
...
- Rewrote search Playwright tests to use `?q=` URL params instead of filling the input and manually dispatching events. The search script already reads `?q=` on page load and calls `runSearch()` automatically once the index is ready; `waitForSelector` then observes the DOM for results — no polling, no event dispatch, no timing hacks. Tests now complete in ~800 ms. (which timed out on slow runners) with a `waitForFunction` loop that re-fires the input event every second until `.eot-result-item` elements actually appear — works regardless of how long the Lunr index takes to build.
