...
- Fixed search Playwright tests timing out in CI: Lunr index build was synchronous and blocked the browser main thread for the full 30 s test timeout on slow GitHub Actions runners (preventing Playwright from even evaluating poll functions). Rewrote the build to process documents in chunks of 30 with `setTimeout(nextChunk, 0)` between batches so the thread yields to the browser.
